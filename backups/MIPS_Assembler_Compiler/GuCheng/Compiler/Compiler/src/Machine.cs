...
namespace Interpreter
{
    ...
    public class Machine
    {
        ...
        public Dictionary<string, Varible> globalMap;
        ...
        {
            System.Threading.Thread t = new System.Threading.Thread(executeTherad);
            t.Start();
            while (Status == MachineStatus.MACHINE_STATUS_PAUSED)
            {
                System.Threading.Thread.Sleep(1);
            }
        }

        private void executeTherad()
        {
        ...
        private void executeInner()
        {
            ...
            while (pc < codeLength)
	        {
		        ...
		        {
		            case OpCode.PUSHNUM:
                        userStack[userStackTop].pointer = code[pc].pointer;
                        userStackTop ++;
			            break;
		            case OpCode.POP:
		            ...
		            case OpCode.RET:
			            ...
			            break;
		            case OpCode.CMP:
                        num = userStack[userStackTop - 2].pointer - userStack[userStackTop - 1].pointer;
                        ...
                        if (cmpState)
			            {
                            userStack[userStackTop - 1].pointer = 1;
			            }
			            else
			            {
                            userStack[userStackTop - 1].pointer = 0;
			            }
			            break;
		            case OpCode.JUMP:
                        cmpState = !(Math.Abs(userStack[--userStackTop].pointer) == 0);
                        if (!cmpState)
                            break;
                        else
                        {
		            ...
		            case OpCode.ADJLOCAL:
			            ...
			            while(n < code[pc].pointer)
			            {
                            userStack[userStackTop] = new Varible(0);
                            ++userStackTop;
				            ++n;
			            }
			            break;
		            case OpCode.NOT:
                        if (Math.Abs(userStack[userStackTop-1].pointer) == 0)
                            userStack[userStackTop - 1].pointer = 1;
			            else
                            userStack[userStackTop - 1].pointer = 0;
			            break;
		            case OpCode.ARITH:
                        ...
                        userStackTop--;
                        userStack[userStackTop - 1].pointer = num;
			            break;
